...
class RelaySocketHandler extends GatewayHandler {
  ...
  setUpRelay(relay) {
    ...
    socket.on("connect", function(){
      this.connectedSockets.push(relay.url);
      socket.emit("onlineLines", this.onlineLines);
    }.bind(this, relay));

    socket.on("disconnect", function(){
      var index = this.connectedSockets.indexOf(relay.url);
      if (index > -1) {
        this.connectedSockets.splice(index, 1);
      }
    }.bind(this, relay));

    this.sockets[relay.url] = socket;
  }
  ...
  */
  emit(title, data) {
    for (var i in this.connectedSockets) {
      this.sockets[this.connectedSockets[i]].emit(title, data);
    }
  }
}
